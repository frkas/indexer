...
/**
 * Given a username, renders out the list of authorities (users and groups) it
 * belongs to
 *
 * Please check
 * src/main/amp/config/alfresco/extension/templates/webscripts/com/findwise/
 * alfresco/authresolve.get.desc.xml to know more about the RestFul interface to
 * invoke the WebScript
 *
 * List of pending activities (or TODOs) - Using JSON libraries (or
 * StringBuffer), render out the payload without passing through FreeMarker
 * template
 */
public class AuthorityResolverWebScript extends DeclarativeWebScript {
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		...
		if (StringUtils.isEmpty(username)) {
			...
			PagingResults<PersonService.PersonInfo> people = this.personService.getPeople("", new ArrayList<QName>(),
					new ArrayList<Pair<QName, Boolean>>(), pagingRequest);
